...
def get_anchors_and_decode(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    #--------------------------------------------------------------------#
    grid_x  = K.tile(K.reshape(K.arange(0, stop=grid_shape[1]), [1, -1, 1, 1]), [grid_shape[0], 1, num_anchors, 1])#最终得到一个形状为 (grid_shape[0], grid_shape[1], num_anchors, 1) 的张量，其中包含了特征图每个位置与先验框的对应网格坐标
